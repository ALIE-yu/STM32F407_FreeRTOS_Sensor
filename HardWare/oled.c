...
#define MY_I2C_GPIO_PORT GPIOE
#define MY_I2C_SCL_PIN GPIO_PIN_13
...
{
	HAL_GPIO_WritePin(MY_I2C_GPIO_PORT, MY_I2C_SCL_PIN, (GPIO_PinState)value);
	i2c_Delay();
}

void OLED_W_SDA(uint8_t value)
{
	HAL_GPIO_WritePin(MY_I2C_GPIO_PORT, MY_I2C_SDA_PIN, (GPIO_PinState)value);
	i2c_Delay();
}
...
void OLED_GPIO_Init(void)
{
	...
	GPIO_InitStructure.Pin = MY_I2C_SCL_PIN | MY_I2C_SDA_PIN;
	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_MEDIUM;
	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_OD; /* 开漏输出 */
	GPIO_InitStructure.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(MY_I2C_GPIO_PORT, &GPIO_InitStructure);
...
void OLED_WriteCommand(uint8_t Command)
{
	...
	OLED_I2C_SendByte(0x00);	// 控制字节，DC位为0，给0x00，表示即将写命令
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	OLED_I2C_Start();		 // I2C起始
	OLED_I2C_SendByte(0x78); // 发送OLED的I2C从机地址
	OLED_I2C_SendByte(0x40); // 控制字节，DC位为1，给0x40，表示即将写数据
	/*循环Count次，进行连续的数据写入*/
	for (i = 0; i < Count; i++)
	{
